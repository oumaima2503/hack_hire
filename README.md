...
- **Secret picture pattern per child**: each child opens only their own world by tapping 4 pictures (🐱 → ⭐ → 🚀 → 🦖) in the Kids' corner (`/kids`).
  ...
  - The pattern is the **only** way into a child's world, even for parents: the parent area has no Play button, and opening a world always switches to child mode first. Parents can **recover** a forgotten pattern: on the Edit page, in parent mode, they type their password again and the pictures show for 30 s. A copy is stored encrypted in `pattern_secret` (key: `PATTERN_SECRET_KEY`, set a Fernet key in production). Parents can also reset the pattern (Edit page, then it is re-created with them in the Kids' corner) and delete a child from the dashboard after a confirmation.
